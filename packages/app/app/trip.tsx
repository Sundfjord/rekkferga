...
import { getTripData, clearTripData } from "@/store/tripStore";
...
// Latest position stored at module level so AppState resume can read it immediately
...
export default function TripScreen() {
  ...
  useEffect(() => {
    const { journey: j, destination: d } = getTripData();
    if (j && d) {
  ...
  // Start location tracking once journey is loaded
  useEffect(() => {
    if (!journey) return;
    ...
    // On app resume, immediately recalculate from latest stored position
  ...
  }, [!!journey]); // Only restart if journey existence changes, not on every departure refresh
